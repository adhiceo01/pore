...
import { createCheckoutSession, completePaymentAction } from '@/app/actions/orders';
...
export default function CheckoutClient({ userLoyaltyPoints, userEmail, userName }: CheckoutClientProps) {
  ...
  useEffect(() => {
    // Load cart
    const savedCart = localStorage.getItem('kore_cart');
    if (savedCart) {
      try {
  ...
  const handlePlaceOrder = async () => {
    ...
    const { orderId, isCash, razorpayOrderId, razorpayKeyId, isSimulation } = response;
